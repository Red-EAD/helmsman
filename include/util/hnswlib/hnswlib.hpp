...
class AlgorithmInterface {
 public:
    ...
    virtual std::vector<std::pair<dist_t, labeltype>>
        searchKnnCloserFirst(const void* query_data, size_t k,uint32_t max_visits, BaseFilterFunctor* isIdAllowed = nullptr) const;
...
AlgorithmInterface<dist_t>::searchKnnCloserFirst(const void* query_data, size_t k,uint32_t max_visits,
                                                 BaseFilterFunctor* isIdAllowed) const {
    std::vector<std::pair<dist_t, labeltype>> result;

    // here searchKnn returns the result in the order of further first
    auto ret = searchKnn(query_data, k, max_visits, isIdAllowed);
    {
        size_t sz = ret.size();
        result.resize(sz);
        while (!ret.empty()) {
